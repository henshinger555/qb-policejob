...
local function UpdateBlips()
    ...
    for _, v in pairs(players) do
        ...
        --print(HasPoliceJob(v.PlayerData.job.name))

        if v and (HasPoliceJob(v.PlayerData.job.name) or HasSupportJob(v.PlayerData.job.name)) and v.PlayerData.job.onduty then
...
local function GetCurrentCops()
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and HasPoliceJob(v.PlayerData.job.name) and v.PlayerData.job.onduty then
            amount += 1
        end
    end
    return amount
...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if HasPoliceJob(Player.PlayerData.job.name) and Player.PlayerData.job.grade.level >= Config.LicenseRank then
...
QBCore.Commands.Add("paylawyer", Lang:t("commands.paylawyer"), {{name = "id",help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if HasPoliceJob(Player.PlayerData.job.name) or Player.PlayerData.job.name == "judge" then
        local playerId = tonumber(args[1])
        local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
...
QBCore.Commands.Add('fine', Lang:t("commands.fine"), {{name = 'id', help = Lang:t('info.player_id')}, {name = 'amount', help = Lang:t('info.amount')}}, false, function(source, args)
    ...
    if HasPoliceJob(biller.PlayerData.job.name) then
        if billed ~= nil then
...
QBCore.Commands.Add("takedna", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if not OtherPlayer or NotPolice(Player.PlayerData.job.name) or not Player.PlayerData.job.onduty then return end
...
QBCore.Functions.CreateUseableItem("moneybag", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or not Player.Functions.GetItemByName("moneybag") or not item.info or item.info == "" or HasPoliceJob(Player.PlayerData.job.name) or not Player.Functions.RemoveItem("moneybag", 1, item.slot) then return end
...
QBCore.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(_, cb)
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and HasPoliceJob(v.PlayerData.job.name) and v.PlayerData.job.grade.level >= 2 then
            retval = true
            break
        end
    end
...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if not Player or not CuffedPlayer or (not Player.Functions.GetItemByName("handcuffs") and NotPolice(Player.PlayerData.job.name)) then return end
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    ...
    if (HasPoliceJob(Player.PlayerData.job.name) or HasSupportJob(Player.PlayerData.job.name)) or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not OtherPlayer or NotPolice(Player.PlayerData.job.name) then return end
